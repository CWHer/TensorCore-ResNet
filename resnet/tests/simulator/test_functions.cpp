...
void naiveMatmul(const float *a, const float *b, float *c, int n, int m, int k) {
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++) {
      float sum = 0;
      for (int l = 0; l < k; l++)
...
TEST(functions, gemm) {
  ...
  for (int i = 0; i < k; i++)
    for (int j = 0; j < m; j++)
      b[i * m + j] = static_cast<float>(i + j * k);
...
TEST(functions, gemm_irregular) {
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < k; j++)
      a[i * k + j] = static_cast<float>(std::sin(i * 2.0 + j * 3.0));
  for (int i = 0; i < k; i++)
    for (int j = 0; j < m; j++)
      b[i * m + j] = static_cast<float>(std::cos(i * 3.0 + j * 2.0));
