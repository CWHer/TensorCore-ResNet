...
public:
    ...
    Tensor forward(Tensor x) override
    {
        ...
        checkCppErrorsMsg(x.sizes().size() != 4, "BatchNorm2dRelu only support 4D input");
        checkCppErrorsMsg(x.sizes()[1] != num_features, "BatchNorm2dRelu input channel size mismatch");
        checkCppErrorsMsg(x.getDevice() != DeviceType::CUDA ||
                              weight.getDevice() != DeviceType::CUDA,
                          "BatchNorm2dRelu only support CUDA");
        ...
        hostBatchNorm2dRelu(input_data, output_data,
                            mean_data, var_data, weight_data, bias_data,
                            eps, batch_size, num_channels, height, width);
    ...
    void printModule(const std::string &prefix) override
    {
        std::cout << prefix << ":BatchNorm2dRelu" << std::endl;
    }
