...
__global__ void deviceMaxPool2dKernel(const float * RESTRICT input_data, int height, int width,
  ...
  auto input_grid_offset = blockIdx.x * height * width;
  auto output_grid_offset = blockIdx.x * out_height * out_width;

  auto row = block_size * threadIdx.y;
      ...
      if (row + i < out_height && col + j < out_width) {
        ...
        for (int x = 0; x < kernel_size; ++x)
          for (int y = 0; y < kernel_size; ++y) {
            int input_row = static_cast<int>((row + i) * stride + x) - padding;
            int input_col = static_cast<int>((col + j) * stride + y) - padding;
...
__global__ void deviceAvgPool2dKernel(const float * RESTRICT input_data, int height, int width,
  ...
  auto output_grid_offset = blockDim.x * out_height * out_width;
  ...
  int output_offset = 0;
  float r = 1.0f / static_cast<float>(kernel_size * kernel_size);
