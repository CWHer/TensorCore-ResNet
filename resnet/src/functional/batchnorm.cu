...
__global__ void deviceBatchNorm2dKernel(float *input_data, float *output_data,
...
template <int block_size>
__global__ void deviceBatchNorm2dReluKernel(float *input_data, float *output_data,
                                            float *mean_data, float *var_data,
                                            float *weight_data, float *bias_data,
                                            float eps, int batch_size, int num_channels,
                                            int height, int width, int total_channels)
{
    ...
    for (int i = 0; i < block_size; i++)
        for (int j = 0; j < block_size; j++)
            if (row + i < height && col + j < width)
            {
                float value = ((input_data[(row + i) * width + col + j] - cur_mean) * r) * cur_weight + cur_bias;
                output_data[(row + i) * width + col + j] = value > 0.0f ? value : 0.0f;
            }
}

void hostBatchNorm2dRelu(float *input_data, float *output_data,
                         float *mean_data, float *var_data,
                         float *weight_data, float *bias_data,
                         float eps, int batch_size, int num_channels,
                         int height, int width)
